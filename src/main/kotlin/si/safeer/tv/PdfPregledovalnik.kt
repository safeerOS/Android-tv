...
object PdfPregledovalnik {
    ...
    private fun jezik(context: Context): String = when (UiText.language.lowercase()) {
        "sl" -> "sl"
        "de" -> "de"
        "es" -> "es-ES"
        "fr" -> "fr"
        ...
        fun shrani(ime: String, base64: String, zeton: String) {
            ...
            Thread {
                try {
                    ...
                    val mapa = si.safeer.tv.cast.HubKrmilnik.mapaZaPrejete(context)
                    val cilj = si.safeer.tv.cast.HubTokovi.enolicnaPot(mapa, varnoIme(ime.ifBlank { d.ime }))
                    cilj.outputStream().use { it.write(podatki) }
                    obvesti(UiText.get(R.string.ui_pdf_saved, cilj.name).ifBlank { "PDF: " + cilj.name })
                } catch (e: Throwable) {
                    android.util.Log.w(TAG, "PDF-ja ni bilo mogoce shraniti: ${e.message}")
                    obvesti(UiText.get(R.string.ui_pdf_save_failed).ifBlank { "PDF save failed" })
                }
            }.start()
        }

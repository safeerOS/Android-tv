...
- **All apps opens calmly and shows where each app lives.** The screen used to redraw for every 18 programs and every device that answered, so it kept moving for a second or two. Now it waits for the devices (like *Programs*) and draws everything at once; opening it again is instant. The top row lists every device by name – this TV, Web, your phone, tablet and computer – instead of one *Computer* for all of them.
...
Slovensko: **Datoteke računalnika urejaš s TV in tablice** (s Safeer Controlom 2.1.0+): zadržan OK → *Preimenuj*, *Premakni v mapo*, *Izbriši* (v **Smeti** računalnika). **Slike:** *Zavrti v levo / desno* – shrani se na računalniku (JPEG brez izgube). **Enako za fotografije na telefonu, tablici in drugem televizorju:** zavrti (ležeča → pokončna z enim pritiskom) ali izbriši (v Smeti naprave). Za tujo fotografijo **Android najprej vpraša lastnika** na tisti napravi – televizor počaka na odgovor in izid pokaže sam; ob zavrnitvi to pove. Za naprave so na voljo le dejanja, ki tam delujejo (brez preimenovanja, premika in odpiranja na računalniku). **Tablica ob prvem zagonu enkrat vpraša za obvestila** – brez njih se nista pokazala ne vprašanje za potrditev ne **deljenje zaslona**; zdaj oboje spet dela, tudi ko je Safeer v ozadju. **Popravek:** pokončne fotografije se kažejo pokončno. **Domača stran Safeer OS:** razdelek se imenuje **Aplikacije Safeer OS**; OK odpre samo, kar si izbral (brez nehotenega odpiranja brskalnika v *Vse aplikacije*); aplikacija se odpre s povečavo iz ploščice namesto črnega zaslona. *Vse aplikacije* se odprejo mirno, naenkrat, in zgoraj po imenih naprav pokažejo, kaj je na kateri napravi. **Središče (hub):** naprava, ki neha brati, ne zadrži več drugih – vsaka povezava ima svojo izhodno vrsto.
